...
- `scripts/promote-play-release.sh [--rollout F] [--yes]` — put an
  ...
  **production** track with the `release.md` notes. Uploads nothing; refuses a
  ...
  `edits.validate` runs before commit; dry run unless `--yes`; `--draft`
  stages a Console-reviewable draft instead of rolling out. **Needs the
  ...
  **even for a `--draft`** (verified 2026-08-27 with `--skip-validate`). Production remains gated on P1.1 (screenshots).
